...
@Keep
public class DysoApplication extends Application {
    private static final String TAG = "DysoApplication";
...
//    private static String targetLibPath;
    ...
    public void onCreate() {
         ...
         * 定义变量
         */
        File sourceLibFile = new File(FileUtil.getFile(this, PathType.ANDROID_DATA), "/libs/");
        targetLibFile = new File(FileUtil.getFile(this, PathType.DATA_DATA_FILES), "/dyso/libs/");
        String apkLibsPath = getFilesDir().getAbsolutePath().concat("/dyso/lib/");
         ...
         * 创建架构文件夹
         */
        Set<String> supportedArchitectures = LibUtil.getSupportedArchitectures(this); // 获取apk内lib文件夹下的全部架构
        for (String arch : supportedArchitectures) { // 仅用于创建对应数量的子文件夹
            ...
            if(!sourceLibArchFile.exists()) {
                ...
                DefaultSettingUtil.readDysoSettings(this, simpleArch); // 初始化各架构的设置
            }
        }
         ...
         * 读取总json，并获取当前选择的架构
         */
        String generalSettingsStr = DefaultSettingUtil.readGeneralDysoSettings(this);
        ...
        try {
            ...
            if (jsonObject.has("architecture")) {
                ...
                if (jsonObject.has("log")) {
                    ...
                    Logcat logcat = new Logcat(this, Constant.LOG_NAME);
                    if (!path.isBlank()) {
         ...
         */
        FileUtil.extractAssetLibFiles(this, Constant.librarys, sourceLibFile);

        /**
         * 加载设置
         */
        String settingsStr = DefaultSettingUtil.readDysoSettings(this, selectedArch);
        if (settingsStr == null) {
                ...
                if (file.isFile() && !fileName.endsWith(".json") && !fileName.endsWith(".txt")) {
                    try {
                        if (!LibUtil.isVaildLib(file.getAbsolutePath())) { // 是动态库才拷贝
                            Log.i(TAG, "Copy ".concat(fileName).concat("failed, this file is not a arm.library"));
                        } else {
//                            File targetFile = new File(targetLibPath.concat(fileName));
         ...
         */
        if (useCustomArch) {
            LibUtil.extractLibsFromApk(this, targetLibFile, LibUtil.getComplexArchName(selectedArch));
        }
         ...
         * 初始化动态库的加载路径
         */
        DynamicSoLauncher.INSTANCE.initDynamicSoConfig(this, targetLibFile.getAbsolutePath().concat(File.separator), new Function1<String, Boolean>() {
            @Override
